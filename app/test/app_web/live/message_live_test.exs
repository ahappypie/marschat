defmodule MarschatWeb.MessageLiveTest do
  use MarschatWeb.ConnCase
  ...
  test "message", %{conn: conn} do
    ...
    assert view
           |> form("#form", message: %{name: "test-user", message: ""})
           |> render_submit() =~ html_escape("can't be blank")
  end
  ...
  test "minimum message length", %{conn: conn} do
    ...
    assert view
           |> form("#form", message: %{name: "test-user", message: "h"})
  ...
  test "handle_info/2", %{conn: conn} do
    ...
    Message.create_message(%{"name" => "test-user", "message" => "hello"})
    # test that the name and the message is displayed
    assert render(view) =~ "<b>test-user:</b>"
