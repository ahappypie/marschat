...
object SaturnDelayActor {
  def props: Props = Props(new SaturnDelayActor)
}

class SaturnDelayActor extends Actor {
  ...
  def delay(timestamp: Long): Int = {
    ...
    val saturn = saturnHelio(jd)
    val d = distance(earth, saturn, jd)
    val lt = lightTime(d)

    val jd2 = jd - lt
    val earth2 = earthHelio(jd2)
    val saturn2 = saturnHelio(jd2)
    val d2 = distance(earth2, saturn2, jd2)
  ...
  def saturnHelio(jd: Double): (Double, Double, Double) = {
    val t = (jd - 2451545.0)/365250.0
    val t2 = t*t
    val t3 = t2*t
    val t4 = t3*t
    ...
    val L0 = calc_series(vsop87.saturn.long.l0, t)
    val L1 = calc_series(vsop87.saturn.long.l1, t)
    val L2 = calc_series(vsop87.saturn.long.l2, t)
    val L3 = calc_series(vsop87.saturn.long.l3, t)
    val L4 = calc_series(vsop87.saturn.long.l4, t)
    val L5 = calc_series(vsop87.saturn.long.l5, t)
    val L = (L0 + L1 * t + L2 * t2 + L3 * t3 + L4 * t4 + L5 * t5)

    val B0 = calc_series(vsop87.saturn.lat.b0, t)
    val B1 = calc_series(vsop87.saturn.lat.b1, t)
    val B2 = calc_series(vsop87.saturn.lat.b2, t)
    val B3 = calc_series(vsop87.saturn.lat.b3, t)
    val B4 = calc_series(vsop87.saturn.lat.b4, t)
    val B5 = calc_series(vsop87.saturn.lat.b5, t)
    val B = (B0 + B1 * t + B2 * t2 + B3 * t3 + B4 * t4 + B5 * t5)

    val R0 = calc_series(vsop87.saturn.rad.r0, t)
    val R1 = calc_series(vsop87.saturn.rad.r1, t)
    val R2 = calc_series(vsop87.saturn.rad.r2, t)
    val R3 = calc_series(vsop87.saturn.rad.r3, t)
    val R4 = calc_series(vsop87.saturn.rad.r4, t)
    val R5 = calc_series(vsop87.saturn.rad.r5, t)
    val R = (R0 + R1 * t + R2 * t2 + R3 * t3 + R4 * t4 + R5 * t5)

    (L,B,R)
  }
  //In AU
  def distance(earth: (Double, Double, Double), saturn: (Double, Double, Double), jd: Double): Double = {
    val x = (saturn._3 * Math.cos(saturn._2) * Math.cos(saturn._1)) - (earth._3 * Math.cos(earth._1) * Math.cos(earth._2))
    val y = (saturn._3 * Math.cos(saturn._2) * Math.sin(saturn._1)) - (earth._3 * Math.sin(earth._1) * Math.cos(earth._2))
    val z = (saturn._3 * Math.sin(saturn._2)) - (earth._3 * Math.sin(earth._2))
